...
# Create the "model_path" folder under "models" if it does not exist
if not os.path.exists(f"models/{model_path}"):
    os.makedirs(f"models/{model_path}")

# Create the "embedding_model" folder under "models" if it does not exist
if not os.path.exists(f"models/{embedding_model}"):
    os.makedirs(f"models/{embedding_model}")
...
tag = 'ada'
if not os.path.isfile(f'./models/{embedding_model}/data/vectorized_{tag}_dataset.pkl'):
    ...
    # Save vectorized dataset to pickle
    with open(f'./models/{embedding_model}/data/vectorized_{tag}_dataset.pkl', 'wb') as f:
        pickle.dump(vectorized_ada_df, f)
else:
    # Load vectorized dataset from pickle
    with open(f'./models/{embedding_model}/data/vectorized_{tag}_dataset.pkl', 'rb') as f:
